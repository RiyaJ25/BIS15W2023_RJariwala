...
As shown by the scatterplot above and with the exception of outliers, it seems that generally having more money does not translate to living longer. 
...
gapminder %>% 
  ...
  pivot_wider(names_from = year,
              values_from = pop) %>% 
  mutate(delta=`2007`-`1952`) %>% #use ` instead of '
  arrange(desc(delta))
```
