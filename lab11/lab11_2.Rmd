...
p #store basic plot as object so that you can play around with the aesthetics
```
...
```{r}
display.brewer.pal(10,"PuOr") #be careful about color blindness, choose palette that allows everyone to see 
```
...
#in website above, choose if data is sequential, diverging, or qualitative and input number of data classes --> choose colorblind safe from left panel
#insert name of color palette into R to use that (4 class paired means number is 4 and name is paired)
...
p+scale_fill_brewer(palette = "BuPu")+
  ...
  labs(title = "Observations by Taxon in Homerange Data",
       ...
       fill= "Trophic Guild")
```
#use same plot and palette for data in report for consistency 
...
```
```{r}
palette2 <- paletteer_d("beyonce::X52")
```
...
```{r}
p2 + scale_fill_manual(values=palette2) #manual doesn't work 
```
```{r}
p2 + scale_color_brewer(palette2) # or you can put packagename:palettename in parenthesis 
#must use brewer not fill because this is points in scatterplot 
```
...
ecosphere %>% 
  ggplot(aes(x=diet, y=log10_mass, fill=migratory_strategy))+ 
  geom_boxplot(alpha=0.4)+ 
  facet_grid(migratory_strategy~.)+   #here we don't specify after period but you can
  theme(axis.text.x = element_text(angle = 60, hjust = 1))+
  labs(title = "Mass by Diet and Migratory Strategy",
       x = NULL,
       y = "Log 10 Mass",
...
## Practice
1. Build a faceted plot that shows a distribution of log10mass by habitat type.
```{r}
ecosphere %>% 
  ggplot(aes(x=log10_mass))+ 
  geom_density()+ 
...
ecosphere %>% 
  ggplot(aes(x=log10_mass))+ 
  geom_density()+ 
  facet_wrap(life_expectancy~habitat) #rows to columns - life expectancy is rows and habitat will be column
```
