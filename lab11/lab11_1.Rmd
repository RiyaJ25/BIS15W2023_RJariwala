...
```{r}
homerange %>% 
  ggplot(aes(y=class, x=log10.mass))+
  geom_boxplot()
```

2. Now, create a plot that fills color for each box by taxonomic class
```{r}
homerange %>% 
  ggplot(aes(y=class, x=log10.mass, fill=class))+
  geom_boxplot()
```
...
```{r}
homerange %>% 
  ggplot(aes(x=class, y=log10.mass, group=taxon, fill=taxon))+  # use group and fill together to see the taxa
  geom_boxplot()
```
...
deserts %>% 
  ...
  summarise(n=n(), .groups='keep') #.groups=keep lets you keep year as grouping variable (without this, there will be an error)
```
In order to see every year, must change year into a factor and then roduce plot: 
...
deserts2 %>% 
  ...
  geom_line()+
  geom_point(shape=5)+ #every point gets a shape, can try diff shape numbers 
...
homerange %>% 
  ...
  geom_histogram(alpha = 0.4, color = "blueviolet", fill = "pink", bins=40)+  #alpha for transparency, specify black outlining and color to fill in bars
...
```{r message=FALSE}
homerange %>%  #to combine both density and histogram, must have both geoms
  ggplot(aes(x=log10.mass)) +
...
homerange %>% 
  ...
  geom_histogram(alpha = 0.4, color = "blueviolet", fill = "pink", bins=40)+
  labs(title = "Histogram Distribution of Homerange")
```
...
```
Say you want to break up into large, medium, small animals:
```{r}
homerange <- homerange %>% 
...
homerange <- homerange %>% 
             mutate(range_category = case_when(log10.hra <= 3.653 ~ "small",
                                              log10.hra>= 4.709 ~ "medium",
                                              log10.hra > 6.016 ~ "large"))
#fix this chunk so that bottom bounds are there too with guidance of interquartile range
```
...
homerange %>% 
  ...
  geom_bar(position="dodge", alpha=0.4, color="black")
```
