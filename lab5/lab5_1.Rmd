...
#these would be better rep as factors

```{r}
mammals %>% mutate_if(is.character, as.factor)
```
#shift+command+m makes a pipe
#SHIFT CONTROL M for PC 
#now all characters are all factors w/o doing this individually one at a time 
...
```
#the & doesn't work because you want the lakeid to be both --> need to use or which is a vertical line |
#comma would work but do not use that 
#then you can make it into a new object like this:
```{r}
fish_subset <- fish %>%
  filter(lakeid == "AL" | lakeid=="AR") %>% 
  filter(radii_length_mm>=2 & radii_length_mm<=4)
...
mammals %>% 
  ...
  arrange(newborn)
```
#if in the newborn column there is -999, i want it to be N/a, and if its anything but -999 i want the old value 
#get rid of all of the -999s in one code 
