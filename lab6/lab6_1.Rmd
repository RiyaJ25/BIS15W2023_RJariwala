...
```
#gives percentage next to end, gives accuracy rounds to one digit 
...
glimpse(msleep)
```
#vore is a character --> we could use tabyl
```{r}
msleep %>% 
  tabyl(vore, order) #finding out which ones have diff vores, for instance carnivore won't have any more than carni
  #this gives answer but its not clean 
```
...
head(msleep)
```
#distinguish summarize and summary
...
msleep %>% 
  ...
  summarize(mean_sleep_lg = mean(sleep_total)) # make a new column that is the mean of sleep total, kind of like mutate. also works for minimum, standard deviation, max, and count 
```
...
msleep %>% 
  filter(order=="Primates") %>% 
      summarize(mean_bodywt = mean(bodywt), 
              ...
              total = n())
```
#make sure to add commas for arithemetic after summarize 
...
```{r}
msleep %>% 
  filter(bodywt > 100) %>% #filter gives too many animals and some of them belong to same genus, we want to know how many distinct genera there are w/ filter of bodyywt over 100
  summarise(n_genera=n_distinct(genus))
```
#There are 9 genera that have a bodywt of greater than 100
...
msleep %>% 
  summarise(n_genera=n_distinct(genus))
```
#summarise and summarize are the same thing
...
```
#There are some carnivores that don't have measured brainweights like panthera. 
...
```
#So now cetacea doesn't have brainweight it's equal to NaN
```{r}
msleep %>% 
  filter(order=="Cetacea") %>% 
  select(order, genus, brainwt)
```
#None of the whales in this data have an estimated brainweight, so NaN is because it can't take mean of nothing. 
