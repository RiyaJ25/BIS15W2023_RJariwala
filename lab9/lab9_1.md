...
```r
malaria_tidy <- read_csv(here("data", "malaria.csv")) 
```
...
##  [5] "District"         "malaria_rdt_0-4"  "malaria_rdt_5-14" "malaria_rdt_15"  
##  [9] "malaria_tot"      "newid"
```

Bolo had the most cases on that date. You can also use slice max to get the highest value. 
...
There is a relationship between mass and homerange. 

### Practice
1. What is the relationship between log10.hra and log10.preymass? What do you notice about how ggplot treats NA's?

```r
ggplot(homerange, mapping=aes(x=log10.hra, y=log10.preymass))+geom_jitter(na.rm=T)
```

![](lab9_1_files/figure-html/unnamed-chunk-15-1.png)<!-- -->
ggplot removes NAs and makes a plot without them. You can also add na.rm=T to avoid the message that indcates that ggplot removed the NAs. 
...
```r
homerange %>% 
  ggplot(aes(x=trophic.guild)) +  #we only want trophic guild so x is categorical and y here is counts --> geom bar is only good for counts
  geom_bar()
```
...
homerange %>% 
  ...
  ggplot(aes(x=common.name, y=log10.mass))+  #here if you changed x to a continuous variable the plot would not work, because then there would be two continuous variables. if you flip x and y the plot will be on its side. 
  geom_col()
```
...
homerange %>% 
  ...
  geom_bar(stat="identity") #if you want to use geom_bar you need to add this line to make same plot as above. you could also just use geom_col like we did above. 
```
