...
```{r, message = FALSE}
spider_list <- lapply(files, read_csv) #applies reading csv across all the files
```

We can view elements in our list with double brackets. Let's view the data for Butte county.
```{r}
spider_list[[3]] #r treats the data sets as vectors, so look at elements and choose using brackets 
```
...
##Practice
#Access the data for Alameda county from spider_list. 
```{r}
spider_list$Alameda
```
...
`inner_join`  
Where did customers 3 and 4 go? Not in data because it only joined common data by customer ID. 
```{r}
...
`left_join`
Where did customer 4 go? Notice the missing data for customer 3.Data doesn't contain 4, and 3 was added but NA because no values for 3 in data set 2. 
```{r}
...
`right_join`
Where did customer 3 go? Notice the missing data for customer 4.Right join added customer 4 to the first data set but there are NA values because 4 doesn't have values for those categories. Right for upper (so 4 over 3?) and left for lower (so 3 over 4?)
```{r}
...
`full_join` 
added everything so 3 and 4 come together with 1 and 2, NA values depending on what categories are filled out and what aren't. 
```{r}
