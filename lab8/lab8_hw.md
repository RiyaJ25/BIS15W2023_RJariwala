...
```
## here() starts at C:/Users/riyaj/OneDrive/Desktop/BIS15W2023_RJariwala-main/lab8
```
...
#These data are not tidy because it contains negative numbers that are not classified as NA and there are columns that should be observations. As it is loaded above, the data is in wide format. 
...
```r
most_polluted <- mean_entero %>% 
  filter(year=="2018") %>% 
  arrange(desc(mean_enterococci_cfu_100ml))
head(most_polluted, 1)
```
...
```
South Maroubra Rockpool has the most pollution. 
