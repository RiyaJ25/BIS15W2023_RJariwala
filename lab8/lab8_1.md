...
```
## [1] "/Users/rsjariwa/Desktop/BIS15W2023_RJariwala-main/lab8"
```
...
# "lab8" before "data2" <- doesn't work because here gives you lab 8. here should put you in main directory not folder. look at where you are and adjust accordingly. 
...
(1) each variable has its own column?  
a, b, c, d are diff drugs which are data, so every variable doesn't have its own column. 
(2) each observation has its own row?  
yes
(3) each value has its own cell?  
yes
...
## # … with 14 more rows
```
#this leaves patient where it is because that's not an issue, names_to makes new column, values_to moves values 
#now we're not violating rules of tidy data, this is the format we need to use to make all plots in R. plotting doesn't work on wide data. 
...
These data are not tidy because the income levels are actually data but they each have their own column. 
...
```
Longer amount of observations but only 5 variables. 
...
These data are not tidy because the days are observations but they are labeled as variables. 
...
qpcr_untidy %>% 
  pivot_longer(
    ...
    names_sep="_", #seperates out the two variables that are in the one column 
