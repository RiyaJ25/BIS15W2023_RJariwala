...
shinyApp(ui, server)
```
#type shiyapp and click tab --> builds for you 
...
names(penguins)
```
#select variables that are continuous for input
#after select Input --> give variable a name "x" --> Select X Variable is what user sees, choices are the four variables that user can choose from 
```{r, eval=F, echo=T}
ui <- fluidPage(
...
```{r, eval=F, echo=T}
ui <- fluidPage(
    selectInput("x", "Select X Variable", choices = c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g"), selected = "bill_length_mm"), #make sure to add comma between selectInputs
...
server <- function(input, output) {
  output$plot <- renderPlot({
    ggplot(data=penguins, aes_string(x=input$x, y=input$y, color="species"))+
      geom_point()+
      theme_light(base_size=18)  #notice how we attach inputs to the data and make a plot 
  })
...
shinyApp(ui, server) 
```
#Can add additional aesthetics in geom_bar: (alpha=0.8, color="black")+
