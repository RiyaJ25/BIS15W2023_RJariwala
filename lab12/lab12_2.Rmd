...
```{r}
ui <- fluidPage(titlePanel("Log 10 Homerange by Taxa"),
                selectInput("x", "Select Taxon", choices=c("birds", "lake fishes", "lizards", "tortoises", "turtles", "mammals", "marine fishes", "river fishes", "snakes"), selected="birds"),
                plotOutput("plot", width="600px", height="500px")
)
...
server <- function(input, output, session) {
  output$plot <- renderPlot({
    homerange %>% 
      ...
      geom_density(fill="red", color="black", alpha=0.6) +
      theme_light(base_size=18)
  })
...
Sneaky Example...app that we built in class but with a lot of aesthetics 
```{r, eval=F, echo=T}
ui <- fluidPage(    
