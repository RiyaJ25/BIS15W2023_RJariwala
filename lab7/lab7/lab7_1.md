...
penguins %>% 
  ...
  summarize(n_penguins=n(), .groups = 'keep') #error message if you don't use this - just use for now 
```
...
```
## # A tibble: 1 × 4
##   mean_bill_length mean_bill_depth mean_flipper_length mean_mass
##              <dbl>           <dbl>               <dbl>     <dbl>
...
```r
penguins %>% 
  count(sex, island) #use count so that NAs appear 
```
...
## 9 <NA>   Torgersen     5
```
The penguins with their sex listed as NA occur in Biscoe, Dream, and Torgerson. 
...
penguins %>%
  group_by(species) %>% 
  summarize(across(c(bill_length_mm, bill_depth_mm, flipper_length_mm), sd, na.rm=T),n_samples=n())
```
...
#Instead of listing all the variables out, you could just use contains("mm")
