...
There are no NAs in the amniota data set, but that may be due to R not recognizing certain values as NA values. If we look at the amniota data set itself, we see that there are -999 values which should be considered NA. 
...
#The amphibio data does show NA values as NA, and the total amount of NAs is 170566 
...
## 3 Reptilia       6040
```
It does not make sense that there are thousands of NAs for classes of birds and reptiles. It would make sense for mammals to have 6040 NAs because they don't lay eggs, so the egg mass variable shouldn't apply to them. 
...
```
I think that the authors used NA to indicate that the organism does not live in that environment. The data set shows 1 for the environment that the amphibian lives in and NA for the other 3 types of environment. They probably did not know how to reorganize the data so that this is more organized. 
