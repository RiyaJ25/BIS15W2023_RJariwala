...
surgery %>% 
  filter(race!="NA") %>% 
  count(race) %>% 
  ggplot(aes(x=race, y=n, alpha=0.4, fill=race))+
  geom_col()+
  labs(title="Count of Participants by Race",
       x="Race",
       y="Number of Participants")
```

![](midterm_2_attempt-2_files/figure-html/unnamed-chunk-8-1.png)<!-- -->
...
surgery %>% 
  filter(gender!="NA") %>% 
  ggplot(aes(x=gender, y=age, alpha=0.4, fill=gender))+
  geom_boxplot()+
...
![](midterm_2_attempt-2_files/figure-html/unnamed-chunk-10-1.png)<!-- -->
...
surgery %>% 
  ...
  ggplot(aes(x=asa_status, alpha=0.4, fill=asa_status))+
  geom_bar()+
  labs(title="Health Status of Participants",
       x="Asa Status",
       y="Number of Patients")
```

![](midterm_2_attempt-2_files/figure-html/unnamed-chunk-11-1.png)<!-- -->

6. (3 points) Create a plot that displays the distribution of body mass index for each `asa_status` as a probability distribution- not a histogram. (hint: use faceting!)

```r
surgery %>% 
  ggplot(aes(x=bmi, na.rm=T, alpha=0.4, fill=asa_status))+
  geom_boxplot()+
  facet_grid(~asa_status)+
  labs(title="BMI compared to Patients' Health Status",
       x="BMI",
       y="Number of Patients")
```
...
![](midterm_2_attempt-2_files/figure-html/unnamed-chunk-12-1.png)<!-- -->
...
surgery %>% 
  ...
  group_by(ahrq_ccs) %>% 
  summarize(mean_rate=mean(ccsmort30rate)) %>% 
...
surgery %>% 
  ...
  group_by(ahrq_ccs) %>% 
  summarize(mean_rate=mean(ccscomplicationrate)) %>% 
  arrange(desc(mean_rate)) %>% 
  head(n=5)
```
...
surgery %>% 
  ...
  labs(title="30-day Mortality Rate Associated with Each Operation Type",
       x="Operation Type",
       y="30-day Mortality Rate")
```

![](midterm_2_attempt-2_files/figure-html/unnamed-chunk-15-1.png)<!-- -->
...
surgery %>% 
  filter(mort30=="Yes") %>% 
  group_by(month) %>% 
  summarize(month, mort30, ccscomplicationrate) %>% 
  count(mort30)
```
...
```r
surgery %>% 
  ggplot(aes(x=month, y=ccscomplicationrate, fill=mort30))+
  geom_boxplot(alpha=0.4)+
  facet_wrap(~mort30)+
  theme(axis.text.x=element_text(angle=75, hjust=1, size=8))+
  labs(title="Comparison of 30-Day Mortaility Rate with Complication Rate by Month",
       x="Month",
       y="30-Day Mortality Rate")+
...
```
## Warning: Removed 29347 rows containing missing values (`stat_boxplot()`).
```

![](midterm_2_attempt-2_files/figure-html/unnamed-chunk-17-1.png)<!-- -->

Please provide the names of the students you have worked with with during the exam: Jaishree Ramamoorthi
