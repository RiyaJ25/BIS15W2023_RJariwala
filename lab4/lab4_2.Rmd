...
object appears in environment panel, could be data matrix or frame, could be a vector, or it could be a stored value/function that you named 
...
#select takes out columns, filter looks in rows (can take out -999 from lab 4.1?)
```{r}
filter(fish, lakeid == "AL")
```
#make sure to have the quotations around AL because it's a category 
...
#here we use ! to omit Alabama variable 
```{r}
filter(fish, lakeid != "AL")
```
...
```
#here we are asking for only observations that are 167 to 175 in length column
...
```
#use between to find values that are between 2.5 and 2.55 (not inclusive?)
...
```
# & is for both conditions being met and or is the line (I) where either observation is met (one or the other)
## Practice  
...
```
new_mammal_data <- select(mammals_new, "genus", "family", "species", "mass", "gestation")
new_mammal_data
5. Use the output from #4 to focus on the family Bovidae with a mass greater than 450000.
```{r}
```
filter(new_mammal_data, family=="Bovidae" & mass>450000)
