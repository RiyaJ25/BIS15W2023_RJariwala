...
#every variable has its own column, every observation has its own row, every value has its own cell. this is how data is organized in the tidyverse, so that the data is all seamless in R.
...
```
#put lakeid and annnumber so R knows that they are not objects, they are variables w/in a data frame
#not having quotes can lead to errors 
...
#The - operator is useful in select. It allows us to select everything except the specified variables. Good for excluding a couple variables.
```{r}
...
names(fish)
```
#pull out variables that have length in their name:
```{r}
...
```
# ^ pull out variables that start with a certain word 
...
# can include regex in cheat sheet for exams 
# pull out all variables that start w a and end with er:
```{r}
...
glimpse(fish)
```
#here we want class that is numeric 
```{r}
...
```
# if you do any.NA it says FALSE --> there are no NAs in the data. so why is the mean of the mass negative? answer is because scientists accidentally inputted -99 instead of NA in the data (didn't leave field blank because mass of animal wasn't measured - do not put 0 for the mass)
...
```
# do not have to change all the names, just the ones w wrong punctuation and capitals
